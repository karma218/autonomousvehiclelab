...
def plot_bboxes(image, boxes, b1, b2, labels=[], colors=[], score=True, conf=None):
  #Define COCO Labels
  ...
  for box in boxes:
    ...
    return image, largest_p1, largest_p2      # Check if any point in the boxes lies within the trapezoid
...
def detect_object(msg):
  ...
  trapezoid_points = np.array([[140, 480], [530, 480], [370, 280], [270, 280]], np.int32)
  ...
  image, p1, p2 = plot_bboxes(image, boxes, b1, b2, score=True, conf=0.50)
  person = False 
  if p1 is not None:
    cb = int((p1[0] + p2[0]) / 2)
    print(p1, p2, cb)
    ...
    if p2[1] > 280 and cb > cf and p1[0] < 530:
        direction = f"left"
    elif p2[1] > 280 and cb < cf and p2[0] > 140:
      direction = f"right"
    else:
      direction = f"straight"
    

  else:
    direction = f"straight"
  return image, direction, person